...
function go_save(){
	...
	// adminProductWrite 클래스 만들고 cos.jar의 multipartRequest를 이용해 파일 업로드하고
	// product 테이블에 레코드 추가해주세요
...
function go_order_save(){
	// 현재 화면에 보여지고 있는 주문들의 체크박스들의 체크된 상태가 몇개나 체크되어있는지 개수를 count 합니다.
	var count=0;
	...
	// count값이 0이면, 더 진행하지 않고 orderList.jsp로 되돌아 갑니다.
	if(count==0){
		alert("주문처리할 항목을 선택해 주세요.");
	}else{ 
	// count값이 1 이상이라면, 현재 폼 안에 있는 체크박스 밸류값들을 갖고, 처리완료로 처리하러 command=AdminOrderSave로 이동
		document.frm.action="shop.do?command=adminOrderSave";
		document.frm.submit();
	}
	// 처리하고 (주문의 result값을 '1' => '2'로 변경) orderList.jsp로 되돌아옵니다.
